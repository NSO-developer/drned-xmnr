        ...
        confd_hkeypath_t *keypath, confd_value_t *newval)
{
    ...
    INFO("ip-mask: Set hook detected.");
    ...
    if (ip.type != C_NOEXISTS && netmask.type != C_NOEXISTS) {
        if (!maapi_exists(maapisock, tctx->thandle, "%s/gw", path)) {
            ...
            INFO("ip-mask: gw for host %s already set.", path);
        }
    }
        ...
        confd_hkeypath_t *keypath)
{
    ...
    WARN("ip-mask: This 'create hook' function  should not be called as create "
        ...
        confd_hkeypath_t *keypath)
{
    ...
    INFO("ip-mask: Remove hook detected, no change done.");
...
static int create_ipv6_hosts(const int th)
{
    ...
    while (mc.n != 0) {
        ...
        if (strlen(CONFD_GET_CBUFPTR(&mc.keys[0])) > 4) {
            // this is to fail the transaction on the device
            INFO("Failing the transaction, name=%s",
                 CONFD_GET_CBUFPTR(&mc.keys[0]));
            goto term;
        }
        if (!maapi_exists(maapisock, th, "/hosts-ipv6{%x}",
                &mc.keys[0])) {
            TRACE("Ipv6 host does not exists, creating...");
            if (CONFD_OK
                    != maapi_create(maapisock, th, "/hosts-ipv6{%x}",
                            &mc.keys[0])) {
...
static int delete_ipv6_hosts(const int th)
{
    ...
    TRACE("Initializing  maapi cursor");
    maapi_init_cursor(maapisock, th, &mc, "/hosts-ipv6");
    maapi_get_next(&mc);
    while (mc.n != 0) {
        trace_confd_val("mc.keys[0] ", &mc.keys[0]);
        if (!maapi_exists(maapisock, th, "/hosts{%x}", &mc.keys[0])) {
            ...
            if (CONFD_OK
                    != maapi_delete(maapisock, th, "/hosts-ipv6{%x}",
                            &mc.keys[0])) {
        ...
        confd_hkeypath_t *keypath)
{
    ...
    if (CONFD_OK != delete_ipv6_hosts(tctx->thandle)) {
           FATAL("Failed to delete ipv6 hosts!");
           goto term;
    }
...
int main(int argc, char *argv[])
{
    ...
    if ((strport = getenv("CONFD_IPC_PORT")) != NULL) {
        port = atoi(strport);
    } else {
        printf("no port\n");
    }
